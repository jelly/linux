...
#define LG_CHARGE_TYPES         (BIT(POWER_SUPPLY_CHARGE_TYPE_STANDARD)	| \
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPES:
		err = set_charge_end_threshold((val->intval == POWER_SUPPLY_CHARGE_TYPE_LONGLIFE)
					       ? 80 : 100);
		if (err)
			return err;
		return 0;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_END_THRESHOLD:
		if (val->intval == 100 || val->intval == 80) {
			err = set_charge_end_threshold(val->intval);
			if (err)
				return err;
			return 0;
		}
