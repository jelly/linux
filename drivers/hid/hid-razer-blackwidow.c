...
static bool no_macro_key_remapping = 0;
module_param(no_macro_key_remapping, bool, 0644);
MODULE_PARM_DESC(no_macro_key_remapping, " on (Y) off (N)");
...
static int razer_blackwidow_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (hdev->type == HID_TYPE_USBMOUSE) {
		...
		ret = hid_hw_raw_request(hdev, 0, buf, RAZOR_BLACKWIDOW_TRANSFER_BUF_SIZE, HID_FEATURE_REPORT, HID_REQ_SET_REPORT);
